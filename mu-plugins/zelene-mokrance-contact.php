...
const ZM_CONTACT_VERSION = '1.0.0';
...
function zm_contact_logos( $field ) {
	...
	foreach ( $ids as $id ) {
		...
		$html .= '<div class="zm-contact__logo">' . wp_get_attachment_image( (int) $id, 'medium', false, array( 'loading' => 'lazy' ) ) . '</div>';
	}
	return $html . '</div>';
}
...
add_shortcode( 'zm_contact_page', function () {
	...
	<div class="zm-contact">
		...
		<section class="zm-contact__about">
			...
			<?php $refs = zm_contact_logos( 'zm_references' ); if ( $refs ) : ?><h3>Referencie</h3><?php echo $refs; endif; ?>
